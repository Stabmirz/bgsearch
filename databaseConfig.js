    ...
    con.query(sql, function (err, result) {
        ...
        if((result.length<1)){
            ...
            con.query(sql, function (err, data) {
               ...
               if(cId!=null && cId!=='null') sql+="customer_id,";
               if(searched_person_id!=null && searched_person_id !=='null') sql+="searched_person_id,";
               if(first_name!=null && first_name !=='null') sql+="first_name,";
               if(last_name!=null && last_name !== 'null') sql+="last_name,";
               if(email!=null && email !== 'null') sql+="email,";
               if(phone!=null && phone!=='null') sql+="phone) VALUES (";
               if(cId!=null && cId !=='null') sql+="'"+cId+"',";
               if(searched_person_id!=null && searched_person_id !=='null') sql+="'"+searched_person_id+"',";
               if(first_name!=null && first_name !=='null') sql+="'"+first_name+"',";
               if(last_name!=null && last_name!=='null') sql+="'"+last_name+"',";
               if(email!=null && email !== 'null') sql+="'"+email+"',";
               if(phone!=null && phone !=='null') sql+="'"+phone+"')";
                con.query(sql, function (err, result) {
                    if (err) throw err;
                });
            });
...
function getDataBySearchedPersonId(id,callback){
    let sql = "SELECT * from searched_person where id='"+id+"'";
    con.query(sql, function (err, data) {
        if (err) throw err;
        return callback(data);
    });
