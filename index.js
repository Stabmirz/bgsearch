...
app.post('/order_details', function (req, res) {
    ...
    fetch('https://'+apiKey+':'+apiSecret+'@netdetective.myshopify.com/admin/api/2020-01/orders/'+orderId+'.json', {
        method: 'GET',
    }).then(function (res) {
        return res.json();
    }).then(function (json) {
        let cId = json.orders.customer.id;
        database.showReport(cId,function(data){
    return res.status(200).send(data);
    });
    });
...
app.post('/getDataBySearchedPersonId/:id', function (req, res) {
    let id = req.params.id;
    database.getDataBySearchedPersonId(id,function(data){
        return res.status(200).send(data);
    })
})